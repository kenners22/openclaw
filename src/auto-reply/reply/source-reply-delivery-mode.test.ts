...
describe("resolveSourceReplyDeliveryMode", () => {
  ...
  it("treats native and slash text commands as explicit replies in groups", () => {
    expect(
      resolveSourceReplyDeliveryMode({
        cfg: emptyConfig,
    ...
    expect(
      resolveSourceReplyDeliveryMode({
        cfg: emptyConfig,
        ctx: { ChatType: "group", CommandSource: "text", CommandBody: "/status" },
      }),
    ).toBe("automatic");
    expect(
      resolveSourceReplyDeliveryMode({
        cfg: emptyConfig,
        ctx: { ChatType: "group", CommandSource: "text", CommandBody: "hello" },
      }),
    ).toBe("message_tool_only");
  });
...
describe("resolveSourceReplyVisibilityPolicy", () => {
  ...
  it("keeps native and slash text command replies visible in groups", () => {
    for (const ctx of [
      { ChatType: "group", CommandSource: "native" },
      { ChatType: "group", CommandSource: "text", CommandBody: "/status" },
    ] as const) {
      expect(
        resolveSourceReplyVisibilityPolicy({
          cfg: emptyConfig,
